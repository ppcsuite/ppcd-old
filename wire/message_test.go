...
func makeHeader(chaincfg wire.BitcoinNet, command string,
	...
	buf := make([]byte, 24)
	binary.LittleEndian.PutUint32(buf, uint32(chaincfg))
...
func TestMessage(t *testing.T) {
	...
	*/
	...
	tests := []struct {
		...
		chaincfg wire.BitcoinNet // Network to use for wire encoding
		bytes  int             // Expected num bytes read/written
	}{
		{msgVersion, msgVersion, pver, wire.MainNet, 124},
	...
	for i, test := range tests {
		...
		nr, msg, _, err := wire.ReadMessageN(rbuf, test.pver, test.chaincfg)
		if err != nil {
	...
	for i, test := range tests {
		// Encode to wire format.
		var buf bytes.Buffer
		err := wire.WriteMessage(&buf, test.in, test.pver, test.chaincfg)
		if err != nil {
			t.Errorf("WriteMessage #%d error %v", i, err)
			continue
		}
		...
		// Decode from wire format.
		rbuf := bytes.NewReader(buf.Bytes())
		msg, _, err := wire.ReadMessage(rbuf, test.pver, test.chaincfg)
		if err != nil {
			t.Errorf("ReadMessage #%d error %v, msg %v", i, err,
				spew.Sdump(msg))
			continue
...
func TestReadMessageWireErrors(t *testing.T) {
	...
	// otherwise accurate.
	badMessageBytes := makeHeader(chaincfg, "addr", 1, 0xeaadc31c)
	...
	discardBytes := makeHeader(chaincfg, "bogus", 15*1024, 0)

	tests := []struct {
		buf     []byte          // Wire encoding
		pver    uint32          // Protocol version for wire encoding
		chaincfg  wire.BitcoinNet // Bitcoin network for wire encoding
	...
	for i, test := range tests {
		...
		nr, _, _, err := wire.ReadMessageN(r, test.pver, test.chaincfg)
...
func TestWriteMessageWireErrors(t *testing.T) {
	...
	tests := []struct {
		...
		// Command too long.
		{badCommandMsg, pver, chaincfg, 0, wireErr, 0},
		// Force error in payload encode.
		{encodeErrMsg, pver, chaincfg, 0, wireErr, 0},
		// Force error due to exceeding max overall message payload size.
		{exceedOverallPayloadErrMsg, pver, chaincfg, 0, wireErr, 0},
		// Force error due to exceeding max payload for message type.
		{exceedPayloadErrMsg, pver, chaincfg, 0, wireErr, 0},
		// Force error in header write.
		{bogusMsg, pver, chaincfg, 0, io.ErrShortWrite, 0},
		// Force error in payload write.
		{bogusMsg, pver, chaincfg, 24, io.ErrShortWrite, 24},
	}
	...
	for i, test := range tests {
		...
		nw, err := wire.WriteMessageN(w, test.msg, test.pver, test.chaincfg)
